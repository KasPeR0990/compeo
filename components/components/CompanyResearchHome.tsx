...
export default function CompanyResearcher() {
  ...
  return (
    <div className="w-full max-w-5xl p-6 z-10 mb-20 mt-6">
      <h1 className="text-4xl pb-5 font-medium opacity-0 animate-fade-up [animation-delay:200ms]">
      ...
      <form onSubmit={handleResearch} className="flex items-center gap-2 mb-4 mx-auto">
        <input
          value={companyUrl}
          onChange={(e) => setCompanyUrl(e.target.value)}
          placeholder="Enter Company URL (e.g., example.com)"
          className="max-w-xs w-full p-2 border-0 border-b-2 border-black focus:border-b-2 focus:border-gray-800 outline-none bg-transparent text-black placeholder-gray-400 text-sm opacity-0 animate-fade-up [animation-delay:600ms]"
        />
        <button
          type="submit"
          className={`flex-shrink-0 px-4 py-1.5 rounded-full text-sm font-semibold shadow-sm transition-colors duration-150 opacity-0 animate-fade-up [animation-delay:800ms] h-9 ml-2 ${
            isGenerating ? 'bg-gray-400 text-white' : 'bg-black text-white hover:bg-gray-900'
          }`}
          style={{ minWidth: 'auto' }}
